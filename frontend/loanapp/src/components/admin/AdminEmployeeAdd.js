...
//       }
//       //check if dob is not empty and is a valid date and is not a future date
//       if(!dob || dob === '' || new Date(dob) > new Date()) {
//         formIsValid = false;
//         newErrors['dob']='Please enter a valid date of birth'
//       }
//       //check if doj is not empty and is a valid date and is not a future date
//       if(!doj || doj === '' || new Date(doj) > new Date()) {
//         formIsValid = false;
...
const AdminEmployeeAdd = ({user}) => {
 ...
 const handleChanges = (e) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("http://localhost:8080/employee/add", userDetails)
      .then((res) => {
        alert("Employee created with Id: " + res.data.body.employeeID);
        navigate("/admin/employee/edit");
        console.log(JSON.stringify(res.data));
