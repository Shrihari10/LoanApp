...
function AdminLoanEdit() {
  ...
  const handleEdit = (loanId) => {
    const filteredLoanCards = loanCards.filter(
      (loanCard) => loanId == loanCard.loanId
    );
        ...
        <Modal.Body>
          ...
          >
            <Form.Group controlId="loanType">
              ...
              <Form.Control type="text" value={editingLoanId} disabled />
            </Form.Group>
            <Form.Group controlId="loanType">
              <Form.Label>Loan Type</Form.Label>
              <Form.Control type="text" value={editingLoanType} disabled />
            </Form.Group>
          ...
          >
            {loanCards.map((loanCard) => (
              <Card
                ...
                variant="outline"
              >
                <CardHeader fontsize="md" width="10em">
                  {loanCard.loanId}
                </CardHeader>
                <CardHeader fontsize="md" width="10em">
                  {loanCard.loanType}
                ...
                <CardFooter paddingInline={10}>
                  <Stack direction="row" spacing={6} align="center">
                    <Button
                      ...
                      size="sm"
                      colorScheme="blue"
                      onClick={() => handleEdit(loanCard.loanId)}
                    >
                      {" "}
                      <EditIcon /> Edit
                    </Button>
                    <Button
                      ...
                      size="sm"
                      colorScheme="red"
                      onClick={() => handleDelete(loanCard.loanId)}
                    >
                      {" "}
                      <DeleteIcon /> Delete
                    </Button>
